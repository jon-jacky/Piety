...
class TextWrapper:
    ...
    wordsep_re = re.compile(
        # COMMENT OUT THESE RE - MICROPYTHON COMPLAINS ValueError: regex too complex 
        #r"(\s+|"  # any whitespace
        #r"[^\s\w]*\w+[^0-9\W]-(?=\w+[^0-9\W])|"  # hyphenated words
        #r"(?<=[\w\!\"\'\&\.\,\?])-{2,}(?=\w))"
        # REPLACE WITH RE FROM wordsep_simple_re below
        r"(\s+)"
    )  # em-dash
...
_whitespace_only_re = re.compile("^[ \t]+$") # , re.MULTILINE) not in MicroPy
